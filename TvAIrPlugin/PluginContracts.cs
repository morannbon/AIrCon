...
{
    // SDK contract: 既存値の順序は互換性維持のため変更しない。新権限は末尾に追加する。
    ReadEpg,
    ReadReservations,
    WriteReservations,
    PreviewAllocation,
    ...
    // SDK contract 読み取りAPI拡張。既存値を壊さず末尾に追加する。
    ReadSystemStatus,
    ReadEpgStatus,
    ReadKeywordRules,
    ReadProgramRules,
    ...
    // SDK contract 番組表投影API。既存値を壊さず末尾に追加する。
    ReadProgramGuideProjection,
    ReadViewerSessions,

    // SDK contract ViewerControl/API投影拡張。既存値を壊さず末尾に追加する。
    ReadViewerTuners,
    ReadViewerControlContracts,

    // SDK contract リリース前API面の明文化。TVTestヘッダ由来の概念をTvAIr抽象契約として読むための権限。
    ReadHostContracts,

    // SDK contract: 汎用Plugin Host Contract。新規プラグインはkindだけでなくcapability/permissionで明示する。
    OpenPage,
    OpenToolWindow,
    UseActionApi,
    UseWindowApi,
...
public sealed class PluginManifest
{
    ...
    /// <summary>SDK contract: 配布元/作者名。権限判定ではなく表示・監査用。</summary>
    public string Vendor { get; set; } = string.Empty;
    /// <summary>SDK contract: プラグインが前提にするTvAIr Plugin Host Contract。例: 1.0.0。</summary>
    public string HostContractVersion { get; set; } = string.Empty;
    /// <summary>SDK contract: プラグインが参照するTvAIrPlugin SDK version。</summary>
    public string SdkVersion { get; set; } = string.Empty;
    /// <summary>SDK contract: kindとは別の機能宣言。新規拡張はここに追加し、kind固定にしない。</summary>
    public IReadOnlyList<string> Capabilities { get; set; } = Array.Empty<string>();
    /// <summary>SDK contract: 汎用manifestの任意タグ。TvAIr本体は未知タグを拒否しない。</summary>
    public IReadOnlyList<string> Tags { get; set; } = Array.Empty<string>();
    /// <summary>SDK contract: host-managed tool window の Form.Icon 用 .ico 名。HTML表示用assetとは別契約。</summary>
    public string Icon { get; set; } = string.Empty;
    /// <summary>SDK contract: host-managed tool window のタイトルバーだけに使う任意表示名。一覧名・メニュー名には使わない。</summary>
    public string ToolWindowTitle { get; set; } = string.Empty;

    /// <summary>SDK contract: プラグイン一覧から直接開く推奨モード。page / toolWindow / browser。</summary>
    ...
    /// <summary>SDK contract: host-managed tool window の最小幅。fallback menu 経路でも本体が強制する。</summary>
    public int ToolWindowMinWidth { get; set; } = 0;
    /// <summary>SDK contract: host-managed tool window の最小高。fallback menu 経路でも本体が強制する。</summary>
    ...
    /// <summary>SDK contract: ハンバーガー/タスクトレイ共通プラグインメニューで実行する既定アクション。toolWindow / page / settings / versionDialog / statusDialog / none。</summary>
...
/// <summary>
/// SDK contract以降の拡張API方針。IPluginContext本体は破壊的変更せず、
...
/// <summary>
/// SDK contract: Phase 2 読み取りAPI拡張。
...
/// <summary>
/// SDK contract: ViewerControl/常駐視聴パネル向け。本体完成済みモジュールの公式投影API。
...
public interface IPluginReadContextV4 : IPluginReadContextV3
{
    ...
    /// <summary>SDK contract: ViewerControl局行用の公式identity投影。viewerStart用tripletはこのAPIを正とする。</summary>
...
/// <summary>
/// SDK contract: リリース前のプラグインAPI面確認契約。
...
public sealed class PluginProgramGuideQuery
{
    ...
    /// <summary>SDK contract: 番組表の放送波フィルタ分類。DisplayGroup互換だが、番組表ボタンの公式投影名。</summary>
...
public sealed class PluginProgramGuideChannel
{
    ...
    /// <summary>SDK contract: 番組表放送波フィルタボタンの分類をそのまま投影する公式field。</summary>
    ...
    public ushort TransportStreamId { get; set; }
    public ushort ServiceId { get; set; }
    /// <summary>SDK contract: NetworkId互換alias。公式fieldはNetworkId。</summary>
    public ushort Nid { get; set; }
    /// <summary>SDK contract: TransportStreamId互換alias。公式fieldはTransportStreamId。</summary>
    public ushort Tsid { get; set; }
    /// <summary>SDK contract: ServiceId互換alias。公式fieldはServiceId。</summary>
    public ushort Sid { get; set; }
    public int ChannelSpace { get; set; }
    public int ChannelIndex { get; set; }
    public string ChannelArgument { get; set; } = string.Empty;
...
/// <summary>
/// SDK contract: ViewerControl局行用の公式identity投影。
...
public sealed class PluginViewerTunerInfo
{
    ...
    // SDK contract: Viewer availability projection for expected-denied handling.
    ...
    // SDK contract: Last viewerStart result for tool-window state rendering.
...
public sealed class PluginUiDescriptor
{
    ...
    public string Description { get; set; } = string.Empty;
    /// <summary>SDK contract: UI単位の任意capability。manifest側Capabilitiesが公式で、ここはUI投影補助。</summary>
    public IReadOnlyList<string> Capabilities { get; set; } = Array.Empty<string>();
    public string Icon { get; set; } = string.Empty;
    public int DisplayOrder { get; set; } = 1000;
    public bool Enabled { get; set; } = true;
    /// <summary>SDK contract: plugin一覧カードの既定起動モード。空なら通常ページ。</summary>
    public string PreferredOpenMode { get; set; } = string.Empty;
    ...
    /// <summary>SDK contract: host-managed tool window の推奨幅。ハンバーガー/トレイ/openWindow共通入口で参照する。</summary>
    public int ToolWindowWidth { get; set; } = 0;
    /// <summary>SDK contract: host-managed tool window の推奨高さ。ハンバーガー/トレイ/openWindow共通入口で参照する。</summary>
    public int ToolWindowHeight { get; set; } = 0;
    /// <summary>SDK contract: host-managed tool window の最小幅。プラグイン名に依存せず共通入口で参照する。</summary>
    public int ToolWindowMinWidth { get; set; } = 0;
    /// <summary>SDK contract: host-managed tool window の最小高さ。プラグイン名に依存せず共通入口で参照する。</summary>
    public int ToolWindowMinHeight { get; set; } = 0;
}
...
public sealed class PluginViewerProfileInfo
{
    ...
    /// <summary>SDK contract: 物理チューナー番号ではなく、放送波を跨いだTVTest枠番号。0は後方互換入力のみ。</summary>
    public int TvTestFrameIndex { get; set; }

    /// <summary>SDK contract: このTVTest枠で選局可能な放送波。例: GR,BSCS。UIは本体projectionに従う。</summary>
...
public sealed class PluginUiContext
{
    ...
    /// <summary>UIプラグインがフォーム/FetchでPOSTする本体Action route。SDK contractの正式名。</summary>
    public string ActionRoute { get; set; } = string.Empty;

    /// <summary>SDK contract互換名。AIrCon等が明示的にPlugin action routeとして検出するための別名。</summary>
    public string PluginActionRoute { get; set; } = string.Empty;

    /// <summary>SDK contract互換名。ActionEndpointと同値のPlugin action endpoint別名。</summary>
    ...
    /// <summary>SDK contract互換名。ActionMethodと同値のPlugin action method別名。</summary>
    public string PluginActionMethod { get; set; } = "POST";

    /// <summary>このUIで本体が受け付ける代表Action名。プラグイン側の出し分け用。</summary>
    public IReadOnlyList<string> SupportedActions { get; set; } = Array.Empty<string>();

    /// <summary>SDK contract互換名。SupportedActionsと同値のPlugin action一覧別名。</summary>
    ...
    /// <summary>SDK contract互換名。ActionTokenと同値のPlugin action token別名。</summary>
    ...
    /// <summary>SDK contract: HTML/JS側が名称揺れなく参照できるAction contract値。</summary>
    ...
    /// <summary>SDK contract: UIプラグインが本体管理の独立/フロートWindowを要求するための正式route。</summary>
    public string WindowRoute { get; set; } = string.Empty;

    /// <summary>SDK contract: WindowRoute互換名。本体管理Window要求のendpoint。</summary>
    public string WindowEndpoint { get; set; } = string.Empty;

    /// <summary>SDK contract: host-managed window state polling endpoint template。{windowId}を実WindowIdへ置換する。</summary>
    public string WindowStateEndpointTemplate { get; set; } = string.Empty;

    /// <summary>SDK contract: host-managed window iframe内RenderHtmlで設定される現在WindowId。</summary>
    public string CurrentWindowId { get; set; } = string.Empty;

    /// <summary>SDK contract: CurrentWindowId互換名。HTML/JS側で名称揺れなく参照するための別名。</summary>
    public string WindowId { get; set; } = string.Empty;

    /// <summary>SDK contract: host-managed window iframe内RenderHtmlでtrue。通常/plugin表示ではfalse。</summary>
    public bool IsHostManagedWindowContent { get; set; }

    /// <summary>SDK contract: 現在Windowのstate endpoint。CurrentWindowIdがある場合のみ設定される。</summary>
    public string CurrentWindowStateEndpoint { get; set; } = string.Empty;

    /// <summary>SDK contract: C# RenderHtml から直接読める現在Window stateの絶対URL。</summary>
    public string CurrentWindowStateUrl { get; set; } = string.Empty;

    /// <summary>SDK contract: 現在Windowのhost URL。CurrentWindowIdがある場合のみ設定される。</summary>
    public string CurrentWindowUrl { get; set; } = string.Empty;

    /// <summary>SDK contract: C# RenderHtml から参照できる現在Window host URLの絶対URL。</summary>
    public string CurrentWindowAbsoluteUrl { get; set; } = string.Empty;

    /// <summary>SDK contract: RenderHtml中にHTTP自己呼び出しせず参照できる現在WindowのalwaysOnTop状態。</summary>
    public bool CurrentWindowAlwaysOnTop { get; set; }

    /// <summary>SDK contract: RenderHtml中にHTTP自己呼び出しせず参照できる現在Windowのrevision。</summary>
    public int CurrentWindowRevision { get; set; }

    /// <summary>SDK contract: RenderHtml中にHTTP自己呼び出しせず参照できる現在Windowのhost alive状態。</summary>
    public bool CurrentWindowHostAlive { get; set; }

    /// <summary>SDK contract: Window要求HTTP method。標準はPOST。</summary>
    public string WindowMethod { get; set; } = "POST";

    /// <summary>SDK contract: Window要求に同送する短寿命token。ActionTokenと同一レンダリング単位で発行される。</summary>
    public string WindowToken { get; set; } = string.Empty;

    /// <summary>SDK contract: 本体が受け付けるWindow action一覧。</summary>
    public IReadOnlyList<string> SupportedWindowActions { get; set; } = Array.Empty<string>();

    /// <summary>SDK contract: HTML/JS側が名称揺れなく参照できるWindow contract値。</summary>
    ...
    /// <summary>SDK contract: TvAIr本体管理Plugin Tool Window hostの能力。WebView2有無、fallback、reuse/state/close同期対応をプラグインが判断するための契約。</summary>
    ...
    /// <summary>SDK contract: ViewerControl mode向け。no-script制約下で本体が安全に扱う行イベント/視聴操作契約。</summary>
    ...
    /// <summary>SDK contract: TvAIr本体がViewing role構成からTVTest枠単位で動的生成したviewer profile一覧。/api/plugins/viewer-profilesと同一projection。</summary>
    public IReadOnlyList<PluginViewerProfileInfo> ViewerProfiles { get; set; } = Array.Empty<PluginViewerProfileInfo>();

    /// <summary>SDK contract: UIで選択可能なviewer profile一覧。autoを含み、selector可視判定と同一projection。</summary>
    public IReadOnlyList<PluginViewerProfileInfo> SelectableViewerProfiles { get; set; } = Array.Empty<PluginViewerProfileInfo>();

    /// <summary>SDK contract: 既定viewer profile。通常はauto。</summary>
    public string DefaultViewerProfile { get; set; } = "auto";

    /// <summary>SDK contract: auto以外の有効TVTest枠profile数。</summary>
    public int EnabledRealViewerProfileCount { get; set; }

    /// <summary>SDK contract: viewer profile selectorを表示すべきかの本体推奨。</summary>
    public bool SelectorVisibleRecommended { get; set; }

    /// <summary>SDK contract: SelectorVisibleRecommendedの明示alias。</summary>
    public bool ViewerProfileSelectorVisibleRecommended { get; set; }

    /// <summary>SDK contract: selector非表示でも最大構成基準の最小幅を維持すべきか。</summary>
    public bool MinWidthInvariantRequired { get; set; } = true;

    /// <summary>SDK contract: RenderHtmlを呼び出した現在のHTTP request path。通常ページ/host-managed tool window directContent共通。</summary>
    public string CurrentRequestPath { get; set; } = string.Empty;

    /// <summary>SDK contract: RenderHtmlを呼び出した現在のHTTP query string。先頭の?を含む。queryなしは空文字。</summary>
    public string CurrentRequestQueryString { get; set; } = string.Empty;

    /// <summary>SDK contract: CurrentRequestPath + CurrentRequestQueryString。</summary>
    public string CurrentRequestPathAndQuery { get; set; } = string.Empty;

    /// <summary>SDK contract: RenderHtml呼び出し時のquery key/value。wave等のUIフィルタ契約用。</summary>
    ...
    /// <summary>SDK contract: RenderHtml内で同梱小型画像等を参照するための同一オリジンasset base URL。例: /plugin-assets/aircon。</summary>
    public string PluginAssetBaseUrl { get; set; } = string.Empty;

    /// <summary>SDK contract: asset配信の詳細契約。許可拡張子、URL形式、sanitizer制約など。</summary>
    ...
    /// <summary>SDK contract: plugin asset URLを安全に組み立てる。assetNameはファイル名のみを想定し、TvAIr本体側のasset endpointへ解決する。</summary>
...
/// <summary>
/// SDK contract: UIプラグインからTvAIr本体へ安全な操作要求を返すためのAction契約。
...
public sealed class PluginActionRequest
{
    ...
    /// <summary>SDK contract: form POSTでJSON画面へ遷移しないための応答モード。json / refreshWindow / html / noContent。</summary>
    public string ResponseMode { get; set; } = "json";

    /// <summary>SDK contract: responseMode=refreshWindow時に再描画するhost-managed window。</summary>
    public string WindowId { get; set; } = string.Empty;

    /// <summary>SDK contract: responseMode=refreshWindow時の更新対象。標準はcontent。</summary>
    public string RefreshTarget { get; set; } = "content";

    /// <summary>SDK contract: responseMode=refreshWindow時にスクロール位置維持を要求する。</summary>
    public bool PreserveScroll { get; set; } = true;

    /// <summary>SDK contract: host-managed tool window再描画後に可視化したい要素ID。CSS selectorではなくidのみ。</summary>
    public string RefreshScrollTarget { get; set; } = string.Empty;

    /// <summary>SDK contract: refreshScrollTargetのスクロール方法。center / nearest / top。</summary>
...
/// SDK contractでは本体host dispatchを主系とし、このinterfaceはSDK上の契約名公開に留める。
/// </summary>
...
/// <summary>
/// SDK contract: UIプラグインがTvAIr本体管理の独立/フロートWindowを要求するためのWindow契約。
...
public sealed class PluginWindowRequest
{
    ...
    /// <summary>SDK contract: refreshAfter/refreshWindow後にhost側で可視化したい要素ID。CSS selectorではなくidのみ。</summary>
    public string RefreshScrollTarget { get; set; } = string.Empty;
    ...
    /// <summary>SDK contract: updateWindow成功後に同一host-managed windowのcontent再描画を本体へ要求する限定契約。</summary>
    public bool RefreshAfter { get; set; } = false;

    /// <summary>SDK contract: 同一 pluginId + routeSegment の既存ツールウィンドウを再利用する。</summary>
    public bool ReuseExisting { get; set; } = false;

    /// <summary>SDK contract: 既存ツールウィンドウ再利用時に前面化する。</summary>
    public bool ActivateExisting { get; set; } = false;

    /// <summary>SDK contract: form POSTでJSON/白紙画面へ遷移しないための応答モード。json / redirect / redirectBack / hostHandled / toolWindow / toolWindowRedirectBack / hostWindow / html / noContent。</summary>
    public string ResponseMode { get; set; } = "json";

    /// <summary>SDK contract: responseMode=toolWindow後にPOST元画面へ戻すための相対URL。未指定時はRefererまたは /plugin/{routeSegment}。</summary>
...
/// <summary>SDK contract: /api/plugins/window/capabilities の応答契約。プラグインはtoolWindowを使うかredirect互換に落とすか判断できる。</summary>
...
/// <summary>SDK contract: /plugin-window/{windowId}/state の拡張応答契約。</summary>
...
/// SDK contractでは本体host-managed window contractを主系とし、このinterfaceはSDK上の契約名公開に留める。
/// </summary>
